...
def build_node(model, node):
    ...
    if not is_array_like(node.output) and node.size_in > 0:
        sig_in = model.Signal(np.zeros(node.size_in), name="%s.in" % node)
    ...
    if node.output is None:
        ...
        sig_out = model.Signal(np.zeros(node.size_out), name="%s.out" % node)
        ...
        sig_out = model.Signal(node.output, name="%s.out" % node)
    else:
        raise BuildError(
